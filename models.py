...
class Cnn14(nn.Module):
    def __init__(self, sample_rate, window_size, hop_size, mel_bins, fmin, 
        ...
        self.spectrogram_extractor_openmic = Spectrogram(n_fft=window_size, hop_length=hop_size, 
            win_length=window_size, window=window, center=center, pad_mode=pad_mode, 
            freeze_parameters=True)

        # Logmel feature extractor
        self.logmel_extractor_openmic = LogmelFilterBank(sr=sample_rate, n_fft=window_size, 
        ...
        # self.fc_openmic = nn.Linear(2048, classes_num, bias=True)

        self.fc1 = nn.Linear(512, 512, bias=True)
        self.fc_monophonic = nn.Linear(512, classes_num, bias=True)
    ...
    def forward(self, input):
        ...
        x = self.spectrogram_extractor_openmic(input)   # (batch_size, 1, time_steps, freq_bins)
        x = self.logmel_extractor_openmic(x)    # (batch_size, 1, time_steps, mel_bins)
        ...
        clipwise_output = torch.sigmoid(self.fc_monophonic(x))
...
class Cnn14_scatter(nn.Module):
    def __init__(self, classes_num, J, Q, audio_length):
        
        super(Cnn14_scatter, self).__init__()
        ...
        self.fc_monophonic = nn.Linear(32, classes_num, bias=True)
